...
public class TransitionTileButton : ActionButton
{
    ...
    public void Confirmed(ENameLayer layer)
    {
        ...
        {
            player.PlaceTo(Board.S.southTransitions[(int)layer].Item2.Item1, Board.S.southTransitions[(int)layer].Item2.Item2);
        }
        else if (tile is WestTransitionTile)
        {
            player.PlaceTo(Board.S.westTransitions[(int)layer].Item2.Item1, Board.S.westTransitions[(int)layer].Item2.Item2);

        }
        else if (tile is NorthTransitionTile)
        {
            player.PlaceTo(Board.S.northTransitions[(int)layer].Item2.Item1, Board.S.northTransitions[(int)layer].Item2.Item2);

        }
        else if (tile is EastTransitionTile)
        {
            player.PlaceTo(Board.S.eastTransitions[(int)layer].Item2.Item1, Board.S.eastTransitions[(int)layer].Item2.Item2);
        }
        else
        {
